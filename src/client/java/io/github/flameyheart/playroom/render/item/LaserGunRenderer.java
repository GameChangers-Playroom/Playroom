...
public class LaserGunRenderer extends GeoItemRenderer<LaserGun> {
	...
	public void render(ItemStack stack, ModelTransformationMode transformType, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight, int packedOverlay) {
		...
		animFrameTick = ClientConfig.instance().reducedMotion.isEnabled("laser_charge") ? 0 : ((int) RenderUtils.getCurrentTick()) - PlayroomClient.ANIMATION_START_TICK.getOrDefault(GeoItem.getId(getCurrentItemStack()), 0);
		...
		if(player.getModel().equals("default")) {
			if(rightArm) {
				...
				if(modelAccurate) {
					...
					playerLeftArm.setPivot(8.7f, 16.0f, 26.0f);
					playerLeftArm.setAngles(1.71f, 2.52f, 0);
	...
	private class ChargeLayer extends GlowingGeoLayer<LaserGun> {
		...
		public void render(MatrixStack poseStack, LaserGun item, BakedGeoModel bakedModel, RenderLayer renderType, VertexConsumerProvider bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
			...
			if (cooldown > 0) {
				rechargeTime = Math.max(item.getCooldownTime(currentItemStack), length);
			}

			if (chargeLevel > 0) {
				// Charge up
				layerAlpha = chargeLevel / 100f;
			} else if (animFrameTick < rechargeTime) {
