...
public class LaserGunRenderer extends GeoItemRenderer<LaserGun> {
    ...
    public void render(ItemStack stack, ModelTransformationMode transformType, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight, int packedOverlay) {
        ...
        ModelTransformationMode mode = rightArm ? ModelTransformationMode.FIRST_PERSON_RIGHT_HAND : ModelTransformationMode.FIRST_PERSON_LEFT_HAND;

        if (transformType == mode) {
            ...
            if (player.getModel().equals("default")) {
                if (rightArm) {
                    ...
                    if (modelAccurate) {
                        ...
                        playerLeftArm.setPivot(8.6952f, 16.34f, 26.761f);
                        playerLeftArm.setAngles(1.6707f, 2.541f, -.1f);
                    }
                } else {
    ...
    class EnergyLayer extends GlowingGeoLayer<LaserGun> {
        ...
        public void render(MatrixStack poseStack, LaserGun item, BakedGeoModel bakedModel, RenderLayer renderType, VertexConsumerProvider bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
            ...
            if ((!item.isCooldownExpired(currentItemStack) && item.getCooldownReason(currentItemStack) == LaserGun.CooldownReason.RELOAD)) {
				return;
			}
